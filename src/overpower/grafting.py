...
def grafted(path: Path, text: str, root_key: str, name: str, value: Mapping[str, JsonValue]) -> str:
    """`text` with `root_key.name` set to `value`, and every other byte untouched.

    Overwriting is unconditional and asks nothing (ADR 0013): a key of the same
    name is replaced, keeping the whitespace that surrounded it, so nothing else
...
def _node(value: JsonValue, layout: _Layout) -> Value:
    ...
    **Objects expand and arrays stay inline**, which is one rule per shape and
    ...
    as a block of fields, an argument list reads as a line.
    """
