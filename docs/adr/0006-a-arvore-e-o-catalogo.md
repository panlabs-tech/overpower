...
> *Emendado em [Conteúdo vendorizado](https://github.com/ThiagoPanini/overpower/issues/45), ao construir a árvore que esta ADR descreve. O nível de tipo **repete-se dentro do framework** — `content/frameworks/<nome>/<tipo>/<artefato>/` —, com o mesmo conjunto fechado de nomes do pool. A redação original tratava o framework como uma pasta opaca, e ela não fecha: o `list --ai-framework` mostra o tipo de cada artefato como prefixo, e esta ADR proíbe entrada de artefato no arquivo escrito, logo a árvore é a **única fonte possível** do tipo. É também o que faz um framework de tipos misturados — o `.specify/` do spec-kit ao lado de skills renderizadas — caber sem mecanismo novo, já que destino é função de (tipo, runtime, escopo). Não muda nenhuma posição desta ADR; corrige o desenho da convenção que ela decidiu.*
>
> *A mesma curadoria fixou o endereço do arquivo escrito, que a decisão tinha deixado implícito: `src/overpower/catalog/catalog.toml`, a raiz irmã de `content/` que o [#11](https://github.com/ThiagoPanini/overpower/issues/11) nomeou. Ele **não ganha portão**, e a razão é a assimetria de falha medida no [#10](https://github.com/ThiagoPanini/overpower/issues/10): conteúdo perdido é silencioso — skill pela metade e ninguém percebe, que é o que P1 e P2 existem para pegar —, catálogo perdido é alto — o bundle some do `list` e o `install` responde que não conhece o nome. O que falha ruidosamente não precisa de verificação.*
>
> *O **formato** daquele endereço mudou em [O arquivo escrito vira YAML](https://github.com/ThiagoPanini/overpower/issues/136): `src/overpower/catalog/catalog.yaml`. A razão é externa a esta ADR — o manifesto que um repositório caseiro federa em `.overpower/catalog.yaml` passa pelo **mesmo leitor** deste arquivo, e é isso que impede um segundo validador de discordar do primeiro ([ADR 0020](0020-o-manifesto-e-yaml-e-a-receita-ao-lado-nao.md)). A receita de MCP na pasta irmã **não** migrou. Nenhuma posição desta ADR se move: o que entra no arquivo escrito continua sendo só o que a árvore não pode saber, e caminho continua nunca.*
...
**A descrição de uma skill vem do próprio `SKILL.md`**, com o custo aceito conscientemente. O `description:` do frontmatter é gatilho de roteador, não texto de vitrine: mediana 179 caracteres, máximo 517, e cortar na primeira frase não resolve — **13 de 22** continuam passando de uma linha. Na tela aprovada em [Protótipo da experiência de terminal](https://github.com/ThiagoPanini/overpower/issues/12), 4 skills ocupam **32 linhas** com a descrição do upstream contra **15** com uma linha curada. O ganho é que embutido e remoto ficam idênticos, sem uma qualidade que só o embutido pode ter.
...
**O `NOTICE` do wheel não vira dado.** Ele continua sendo o arquivo de texto que o [#11](https://github.com/ThiagoPanini/overpower/issues/11) decidiu — escrito à mão na curadoria, carregando repo, SPDX, titular e ref por origem vendorizada. O programa nunca o lê, e o `list` não mostra origem.

**A regra explica a assimetria entre MCP e hook** que o [#11](https://github.com/ThiagoPanini/overpower/issues/11) mediu sem nomear. *"MCP é só contrato; hook é contrato e árvore"* deixa de ser fato solto e vira consequência: MCP não tem árvore, logo é **inteiro** entrada escrita; hook tem árvore e uma parte que a árvore não sabe, logo é os dois. A árvore de um hook é descoberta em `content/pool/hooks/<nome>/` como todo o resto, e a entrada casa com ela **pelo nome** — o mesmo mecanismo que um bundle usa para nomear artefatos do pool. Nenhum campo aponta para a árvore; se apontasse, seria caminho.
