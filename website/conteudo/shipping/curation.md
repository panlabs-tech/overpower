...
A graft — an MCP server — reads the tooling clause differently, and without that difference the whole class would be stillborn: nearly every stdio server launches through `uvx`, `npx` or `docker`, and refusing anything that needs external tooling would refuse all of them. The distinction is what actually lands. A copy puts content on disk that only works with some tool, and if that tool is missing the user discovers overpower's own defect after the fact. A graft puts nothing but a declaration on disk — the tooling belongs to the server being described, not to overpower. So instead of a blanket requirement, the recipe names what it needs as a precondition, and overpower checks that precondition itself before writing, refusing with exit `3` and naming exactly what is missing rather than leaving a broken configuration behind. A recipe still fails this gate when it needs something and never says so.
