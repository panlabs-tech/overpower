...
**Emenda ([#100](https://github.com/ThiagoPanini/overpower/issues/100)): numa linha mista, a recusa é por runtime, não mais pela linha inteira — e não precisa de recusa nova.** `--skill a --mcp b --runtime cursor,claude-code` costumava morrer inteira se `cursor` não tivesse documento de MCP, mesmo `cursor` tendo linha de skills e `claude-code` tendo as duas. Medido: isso recusava um install em que nada estava errado com `cursor` na classe que ele *tem*. A régua ficou: um runtime com linha numa classe e não na outra recebe o que tem, e o que não recebeu vira `SkippedClass`, nomeado na tela em exit 0 — nunca em silêncio (`_skipped_classes_of`, que substitui as duas chamadas de `plan_for` numa linha mista). Não nasceu recusa irmã para *"linha em nenhuma das duas"*: o parágrafo abaixo já garante que esse runtime nunca chega até aqui — `_selected_runtimes` recusa antes, pela união das duas tabelas (ADR 0009). A frase *"as duas recusam a linha inteira"* do parágrafo acima ficou verdadeira só para a linha de classe única.

**A escopo continua valendo, e agora sobre a união.** À época, `--runtime vscode --global` saía 3 com a mensagem da ADR 0009: não havia documento de MCP em escopo de máquina e não há diretório de skills em lugar nenhum. **O primeiro membro dessa conjunção caiu em [#81](https://github.com/ThiagoPanini/overpower/issues/81)** — `vscode --global` hoje aterrissa no `mcp.json` do perfil de usuário. A consequência não mudou, só o exemplo: quem sai 3 agora é `--runtime eve --global --skill <x>`, e a regra continua sendo *a união decide, e o que a união não alcança recusa a linha inteira*.
