...
>
> *O **formato** daquele endereço mudou em [O arquivo escrito vira YAML](https://github.com/panlabs-tech/overpower/issues/136): `src/overpower/catalog/catalog.yaml`. A razão é externa a esta ADR — o manifesto que um repositório caseiro federa em `.overpower/catalog.yaml` passa pelo **mesmo leitor** deste arquivo, e é isso que impede um segundo validador de discordar do primeiro ([ADR 0020](0020-o-manifesto-e-yaml-e-a-receita-ao-lado-nao.md)). A receita de MCP na pasta irmã **não** migrou. Nenhuma posição desta ADR se move: o que entra no arquivo escrito continua sendo só o que a árvore não pode saber, e caminho continua nunca.*
