...
Decidido em [Runtime sem caminho global em escopo global: recusa, pulo ou exit code](https://github.com/ThiagoPanini/overpower/issues/29). A tabela e o `None` que a dispara vieram de [Tabela de caminhos de runtime](https://github.com/ThiagoPanini/overpower/issues/27).

## A régua dos códigos, agora completa

A [Busca remota](https://github.com/ThiagoPanini/overpower/issues/25) já tinha estreitado a régua do [#8](https://github.com/ThiagoPanini/overpower/issues/8) para o caso dela. Esta decisão a fecha para o produto inteiro:
...
A leitura era construir *"o conjunto válido de `--runtime` é função do escopo, logo em global `eve` é valor inválido, logo exit 2"* — reusando o precedente que o [#8](https://github.com/ThiagoPanini/overpower/issues/8) já aplicou a `--runtime` inexistente, sem inventar semântica nova. **Perdeu por duas razões, e nenhuma é de gosto.**
...
**Nota ([#100](https://github.com/ThiagoPanini/overpower/issues/100)):** esta leitura foi emprestada por `NoMcpDocumentError` para um **segundo eixo** — classe, não escopo — quando a [ADR 0017](0017-o-eixo-de-enxerto-tem-tabela-propria-de-runtime.md) nasceu. Nessa emenda, o eixo de classe deixou de vetar a linha inteira numa linha mista: um runtime com linha numa das duas classes não é mais varrido pelo gap na outra. Isso não pediu recusa nova — a **união** que este parágrafo já reserva ao eixo de escopo (*"um runtime sem destino de nenhuma classe"*) é exatamente a garantia que faz o eixo de classe dispensar uma: `_selected_runtimes` já recusou, por escopo, todo runtime sem linha em nenhuma das duas tabelas, então quem sobra para o eixo de classe checar tem, no mínimo, uma. O que esta ADR decide — a **escopo** é função do par (runtime, escopo), e onde a união das duas tabelas não alcança, a linha inteira morre — continua de pé e sem emenda; só o empréstimo em ADR 0017 mudou de forma.

**Nota ([#156](https://github.com/ThiagoPanini/overpower/issues/156)):** os `76`/`74` da abertura são o tamanho de `runtimes_in(scope)` — só a tabela de skills. O que `--runtime` de fato aceita, por escopo, é a união com `mcp_runtimes_in(scope)` que a Nota do #100 acima já usa como fronteira: medida na 0.27.1, **77** em projeto e **75** em global, com `vscode` (ADR 0018) como o único membro que a união acrescenta nos dois — projeto perde só isso e por isso fecha com `known_runtimes()`; global ainda perde `eve` e `promptscript` por cima. Nenhuma das três metades do topo desta ADR muda: a régua de exit codes é a mesma, e os dois nomes continuam sendo os dois sem destino global.
