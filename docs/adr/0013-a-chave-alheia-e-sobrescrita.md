...
A alternativa recomendada era **exit 3 pedindo `--force`**, reusando a forma que o produto já aplica a destino existente em escopo global ([história 47](https://github.com/ThiagoPanini/overpower/issues/35)) — ali a razão declarada é *"não há git para desfazer"*.

Os alvos, aliás, discordam entre si, e a discordância foi medida em [Formatos de configuração de MCP por runtime](https://github.com/ThiagoPanini/overpower/issues/17): o **Claude Code recusa** duplicata com exit 1 e sem `--force`; o **VS Code e o Codex sobrescrevem calados**. Não havia comportamento nativo a imitar — só dois campos opostos para escolher.

**O dev escolheu sobrescrever, e a razão é coerência**: a escrita incondicional é a mesma decisão em todo o produto ([#9](https://github.com/ThiagoPanini/overpower/issues/9)), e um segundo regime de colisão obrigaria o usuário a saber de que classe é o artefato antes de prever o que o comando faz.
...
**Emenda ([#125](https://github.com/ThiagoPanini/overpower/issues/125)): chave *duplicada* não é colisão — é arquivo quebrado, e recusa.** Esta ADR governa a chave que o usuário tem **uma vez** e que o overpower substitui. A mesma chave escrita **duas** vezes não é a decisão de ninguém: a RFC 8259 § 4 declara o resultado *unpredictable*, e o produto media entre um documento e um leitor sem poder saber o que o documento diz. Medido, `json.loads` e `JSON.parse` resolvem os dois pela **última** ocorrência enquanto o enxerto aterrissa na **primeira** — sobrescrever ali não é a coerência que esta ADR comprou, é exit 0 sobre um desfecho que o runtime não vê. Era também uma **evasão**: um `mcpServers` que é string sai 3 sozinho e saía 0 escondido atrás de uma raiz duplicada. A recusa é estreita por construção — vale para cada chave que o próprio enxerto **procura** (a raiz, o nome do servidor dentro dela, e a identidade de um elemento de `inputs[]`) e para nenhuma outra; duplicata em qualquer outro lugar do documento continua não sendo assunto do overpower.
